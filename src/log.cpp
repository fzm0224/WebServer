...
extern char message_log[64];
extern char warning_log[64];

/* 日志文件初始化,也可以通过msgLogOpen进行初始化 */
int msgInit(char *pName)
{
	if (msgLogOpen(pName, message_log, LOG_POSTFIX_MESS, warning_log, LOG_POSTFIX_WARN) == 0)
	{
...
int msgLogOpen(char *ident, char *mpre, const char *mdate, char *wpre, const char *wdate)
{
	...
	if ((!msgfile) && (*mpre))
	{
		...
		{ 
			/* 如果没有应该把目录建上 */
			printf("logfile dir not exist! openfilename=%s\n", openfilename);
			return -1;
		}
		setlinebuf(msgfile);
	}
	...
	if ((msgfile) && (wanfile))
	{
		...
		/* 默认信息输出时间格式 MM-DD HH24:MI:SS */
		strcpy(msgdatefmt, "%m-%d %H:%M:%S");
		/* 默认告警输出时间格式 MM-DD HH24:MI:SS */
